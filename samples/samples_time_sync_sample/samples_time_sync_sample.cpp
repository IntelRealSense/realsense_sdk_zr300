...
rs::utils::smart_ptr<samples_time_sync_interface> samples_sync(nullptr);
...
int main(int argc, char* argv[])
{
    ...
    samples_sync = rs::utils::samples_time_sync_interface::create_instance(streams, motions, device->get_name());
...
void cout_sample(correlated_sample_set& sample)
{
    ...
    std::cout << "Received correlated set "<< correlated_frame_counter++ << " of synced frames. Color TS: " << sample.images[static_cast<uint8_t>(stream_type::color)]->query_time_stamp() <<
              " Depth TS: " << sample.images[static_cast<uint8_t>(stream_type::depth)]->query_time_stamp() <<
              " Gyro TS: "<< sample.motion_samples[static_cast<int>(motion_type::gyro)].timestamp <<
              " Accel TS: "<< sample.motion_samples[static_cast<int>(motion_type::accel)].timestamp <<
              " Fisheye TS: " << sample.images[static_cast<uint8_t>(stream_type::fisheye)]->query_time_stamp() << endl;
}
