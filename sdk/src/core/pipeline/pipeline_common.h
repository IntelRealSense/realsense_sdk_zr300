...
namespace rs
{
    namespace core
    {
        ...
        public:
            ...
            virtual status query_available_config(uint32_t index, pipeline_common_interface::pipeline_config & available_config) const override;
            virtual status set_config(const pipeline_common_interface::pipeline_config & pipeline_config) override;
            virtual status query_current_config(pipeline_common_interface::pipeline_config & current_pipeline_config) const override;
        ...
        private:
            ...
            status query_current_config_internal(pipeline_common_interface::pipeline_config & current_pipeline_config) const;
            status get_intersecting_modules_config(std::vector<pipeline_common_interface::pipeline_config> & intersecting_modules_configs) const;
            status filterout_configs_unsupported_the_context(std::vector<pipeline_common_interface::pipeline_config> & configs) const;
